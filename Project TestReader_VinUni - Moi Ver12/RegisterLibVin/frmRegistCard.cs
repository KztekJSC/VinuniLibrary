...
namespace RegisterLibVin
{
    public partial class frmRegistCard : Form
    {
        ...
        private void ShowData()
        {
            ...
            {
                string studentID = "";
                string str1 = $"select * from tblRegister where CardNumber = '{maThe}'";
        ...
        private void UpdateRegisterQLSV()
        {
            // Nếu 2 bản được ghi vào 1 thẻ -> Update bản 1 về ko đăng ký 
            string a = $"select * from tblQLSV where CardID = '{txbCardID.Texts}' and IsRegister = '1'";
            DataTable dtA = ConnectSQL.dbConnection.FillData(a);
            if (dtA != null && dtA.Rows.Count > 0)
            {
                string b = $"update tblQLSV set IsRegister = '0' where CardID = '{txbCardID.Texts}' and IsRegister = '1'";
                ConnectSQL.dbConnection.ExecuteCommand(b);
            }
        }
        ...
        private void CreatSQL_tblRegister()
        {
            if (isbtnWrite)
            {
                ...
                string strRegister = $"insert into tblRegister values ('{CardID}', '{studentID}', '{txbCardID.Texts}', '{MSV}', '{datetime}', N'{userFullName}', '{isDelete}')";
        ...
        private void CreatSQL_Read()
        {
            ...
            string cardNumber = txbCardID.Texts;
            string cardNo = "No1";
        ...
        private bool CardID()
        {
            ...
            if (isbtnWrite)
            {
                ...
                {
                    MessageBox.Show("Thông tin này hiện chưa có trong danh sách, Mời thêm vào danh sách trước khi ghi", "", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
                    result = false;
                    isStopMidWay = true;
                }
...
*/
                ...
                if (port_Card.BytesToRead == 3)
                {
                    ...
                    if (compareTwoByte(fourFirstByte, successfulCardID))
                    {
                        ...
                        txbCardID.Invoke(new Action(() =>
                        {
                            ...
                            txbCardID.Texts = ByteArrayToString(nameCard);
        ...
        private void btnHalt_Leave(object sender, EventArgs e)
        {
        ...
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("SELECT [STT],[Surname],[Name],[Email],[StudentCode],[PhoneNumber],[CardID],[Note],[IsDelete],[IsRegister] ");
