...
namespace RegisterLibVin
{
    ...
    public class LogHelper
    {
        ...
        #region: LOG
        public static void LogUser(string message, string Description, string startUpPath, EM_LogType logType = EM_LogType.CustomerLog, int limitLength = 0)
        {
            string InitPath = GetSavePath(logType, startUpPath);
            string content = GetUserLogContent(logType, message, Description);
            lock (lockObject)
            {
                try
                {
        ...
        public static string GetUserLogContent(EM_LogType logType, string page, string description)
        {
            ...
            string content = $"<b style=\"color: {color}; \">{logType.ToString()}</b>{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff")}<br>" +
                               $"<b style=\"color: {color}; \">Message: </b>{page}" +
                               $"<b style=\"color: {color}; \">Description: </b>{description}" +
                               "<hr style=\"border: none; border-top:1px dashed #f00;color:black;background-color:#fff;height:1px;width:50%;\"><br>";
            //string content = logType.ToString() + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") +
            //                 "\r\n <b>Function Name: " + functionExcecute.FunctionName +
            //                 "\r\n File Name: " + functionExcecute.FileName +
            ...
            SaveSQL(page, description);
            return content;
        }

        private static void SaveSQL(string page, string description)
        {
            ...
            string strLog = $"insert into tblLog values('{LogID}','{time}',N'{userName}',N'{page}',N'{description}',N'{PCName}')";
            ConnectSQL.dbConnection.ExecuteCommand(strLog);
        }
