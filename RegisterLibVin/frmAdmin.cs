...
namespace RegisterLibVin
{
    public partial class frmAdmin : Form
    {
            ...
            if (txbFullName.Texts != "" && txbAcount.Texts != "" && txbPass2.Texts != "" && txbPass.Texts != "")
            {
                if (txbPass.Texts == txbPass2.Texts)
                {
                    ...
                    else
                    {
                        ...
                        {
                            string quyen = "Admin";
                            string query = $"insert into tblUser values (NEWID(),N'{txbFullName.Texts}','{Filter(txbAcount.Texts)}','{Filter(txbPass.Texts)}','{quyen}', '0', '0')";
        ...
        private void LoadPCSever()
        {
            if(Properties.Settings.Default.PathBackup != "")
            {
                txbPath.Texts = Properties.Settings.Default.PathBackup;
            }
            if (Properties.Settings.Default.NameFolderShare != "")
            {
                txbFolderShare.Texts = Properties.Settings.Default.NameFolderShare;
            }
            if (Properties.Settings.Default.PCName != "")
            {
                lblPCName.Text = Properties.Settings.Default.PCName;
            }
        }

        private void LoadImportExcel()
        {
        ...
        {
            string cp = Environment.MachineName;
            txbNamePC.Texts = cp;
        ...
        private void txbNamePC__TextChanged(object sender, EventArgs e)
        {
        ...
        private void btnPathBackup_Click(object sender, EventArgs e)
        {
            ...
            {
                txbPath.Texts = path.SelectedPath;
        ...
        {
            if (string.IsNullOrEmpty(txbNamePC.Texts))
            {
                MessageBox.Show("Chưa tìm thấy tên máy chủ", "Notice", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
                return;
            }
            if (string.IsNullOrEmpty(txbPath.Texts))
            {
                MessageBox.Show("Chưa Lấy thông tin Path Backup", "Notice", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
                return;
            }
            if (string.IsNullOrEmpty(txbPath.Texts))
            {
                MessageBox.Show("Chưa Lấy thông tin Path FolderShare", "Notice", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
                return;
            }
            // Save PC Name
            Properties.Settings.Default.PCName = txbNamePC.Texts;
            lblPCName.Text = Properties.Settings.Default.PCName;
            // Save File Path
            Properties.Settings.Default.PathBackup = txbPath.Texts;
            // Save File FolderShare
            Properties.Settings.Default.NameFolderShare = txbFolderShare.Texts;
